function push_tile(x, y, z, size, weightFactor) {
    xratio = (x + tilesize / 2) / tilesize;
    yratio = (y + tilesize / 2) / tilesize;
    zratio = (z + tilesize / 2) / tilesize;
    // c = color(255 * xratio, 255 * yratio, 255 * zratio);//, 20),
    H = zratio * 1.1;
    S = yratio * 1.1;
    L = xratio * 1.1;
    colorMode(HSB, 1);
...
function make_tiles(step, tilesize, weightFactor) {
    ...
    // for (let x = -tilesize + (step / 2); x < tilesize; x = x + step) {
    //     for (let y = -tilesize + (step / 2); y < tilesize; y = y + step) {
    //         for (let z = -tilesize + (step / 2); z < tilesize; z = z + step) {
